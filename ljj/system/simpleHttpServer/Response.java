...
public class Response {
    ...
    public void sendResource() {
        ...
        try {
            // 错误请求, url读取不出来, 一般 是空请求, chrome的插件会导致发送空请求.
            if (request.getUri() == null) {
                String errorMessage = "HTTP/1.1 444 REQUEST IS NULL \r\n"
                        + "Content-Type: text/html \r\n"
                        + "Content-Length: 24 \r\n"
                        + "\r\n"
                        + "<h1>Request Is Null</h1>";
                System.out.println(errorMessage);
                output.write(errorMessage.getBytes());
                return;
            }
            System.out.println(SimpleHttpServer.FileRoot.isDirectory());
            System.out.println(Arrays.toString(SimpleHttpServer.FileRoot.list()));
            System.out.println("uri=" + request.getUri());
            File file = new File(SimpleHttpServer.FileRoot, request.getUri());
